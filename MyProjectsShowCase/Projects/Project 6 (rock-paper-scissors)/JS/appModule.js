...
const window =
  '<header class="rules-header position-absolute">' +
  '<div class="d-flex justify-content-between">' +
  "<h2>RULES</h2>" +
  '<span><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"><path fill="#3B4262" fill-rule="evenodd" d="M16.97 0l2.122 2.121-7.425 7.425 7.425 7.425-2.121 2.12-7.425-7.424-7.425 7.425L0 16.97l7.425-7.425L0 2.121 2.121 0l7.425 7.425L16.971 0z" opacity=".25"/></svg></span>' +
  "</div>" +
  "</header>" +
  '<div class="game-rules position-absolute">' +
...
function createRulesWindow() {
  ...
  setTimeout(() => {
    w.removeAttribute("hide");
    r.removeAttribute("hide");
  }, 0);
}
function closeRulesWindow() {
  const r = document.querySelector("div.game-rules-box");
  const w = document.querySelector("div.msg");
  r.setAttribute("hide", "");
  w.setAttribute("hide", "");
  setTimeout(() => {
    r.remove();
...
function randomChoices(min, max) {
  return Math.floor(Math.random() * (max - min) + min);
}
function myDecision(data) {
  const triangle = document.querySelector(".triangle");
  const sec = document.querySelector("section");
  const span = createElement("span", "class", `my-${data}`);
  const cont = createElement("div", "class", `my-parent-${data} pc`);
  const el = createElement("div", "class", `my-${data}`, svgs[data]);
  cont.append(span);
  cont.append(el);
  sec.append(cont);
  const userText = createElement("p", "class", "user", "You");
  const pcText = createElement("p", "class", "pc", "PC");
  const myPick = document.querySelector("[choosed]>div");
  const pcPick = document.querySelector(".pc>div");
  userText.setAttribute("hide", "");
  pcText.setAttribute("hide", "");
  myPick.append(userText);
  pcPick.append(pcText);
  triangle.setAttribute("hide", "");
  cont.setAttribute("hide", "");
  setTimeout(() => {
    userText.removeAttribute("hide");
    pcText.removeAttribute("hide");
  }, 300);
  setTimeout(() => {
    cont.removeAttribute("hide");
  }, 700);
}
function takeAction() {
  const arr = ["r", "s", "p"];
  const randNum = [randomChoices(0, len(arr))];
  return arr[randNum];
}
function gameLogic(userChoice, pcChoice) {
  if (userChoice === "s") {
    if (pcChoice === "r") return "lose";
    else if (pcChoice === "p") return "win";
  } else if (userChoice === "r") {
    if (pcChoice === "s") return "win";
    else if (pcChoice === "p") return "lose";
  } else if (userChoice === "p") {
    if (pcChoice === "r") return "win";
    else if (pcChoice === "s") return "lose";
  }
  if (pcChoice === userChoice) return "draw";
}
function scoring(scoring) {
  let score = document.querySelector("div.score span.score");
  setTimeout(() => {
    score.innerText = scoring;
  }, 1000);
}
function playAgainBtn(playerStatus) {
  let gameResult;
  let sec = document.querySelector("section.choose-area");
  let pac = createElement(
    "div",
    "class",
    "play-again-cont d-flex justify-content-between align-items-center flex-column"
  );
  let pab = createElement(
    "span",
    "class",
    "play-again-btn d-flex justify-content-center align-items-center bg-light text-danger",
    "PLAY AGAIN"
  );
  playerStatus !== "draw"
    ? (gameResult = createElement(
        "h2",
        "class",
        "game-result",
        `You ${playerStatus.toUpperCase()}`
      ))
    : (gameResult = createElement(
        "h2",
        "class",
  ...
  pac.setAttribute("hide", "");
  pac.append(gameResult);
  pac.append(pab);
  sec.append(pac);
  setTimeout(() => {
    pac.removeAttribute("hide");
  }, 1300);
}
function addLightBackgroundToWinner() {
  const div = createElement("div", "class", "cont position-absolute");
  const span1 = createElement(
    "span",
    "class",
    "circle-one bg-light position-absolute"
  );
  const span2 = createElement(
    "span",
    "class",
    "circle-two bg-light position-absolute"
  );
  const span3 = createElement(
    "span",
    "class",
    "circle-three bg-light position-absolute"
  );
  div.append(span1);
  div.append(span2);
  div.append(span3);
...
/* 
  1-x Make the game logic
  2-x Make the win and lose background
  3-x Make the 'Play again' button
  4-x Make the score dynamic
  5-x Test all the application
  6-x Double check
  7- Add this project to "Projects_done" folder
  8- Edit the path to my module
  9- Take screen shots
  10- Add a new project to my projects show case
  11- Remove all comments
*/
