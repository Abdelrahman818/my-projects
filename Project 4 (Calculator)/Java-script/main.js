...
for (i in keysArr) {
  ...
  } else if (keysArr[i] === "RESET") {
    span.setAttribute("class", "reset");
  } else if (keysArr[i] === "=") {
    span.setAttribute("class", "enter");
  } else {
...
enter.onclick = () => {
  ...
  } else screen.innerHTML = eval(op);
  if (Number.isInteger(eval(op))) addingComma();
};
...
function switching() {
  switch (theme) {
    case 1:
      if (page.hasAttribute("three")) {
        page.removeAttribute("three");
      }
      page.setAttribute("one", "");
      break;

    case 2:
      if (page.hasAttribute("one")) {
        page.removeAttribute("one");
      }
      page.setAttribute("two", "");
      break;

    case 3:
      if (page.hasAttribute("two")) {
        page.removeAttribute("two");
      }
