function len(param) {
  let i = 0;
  while (true) {
    if (param[i] === undefined) break;
    i++;
  }
  return i;
}
function strRepeat(str, repeates, isSpace = false, spaceType = " ") {
  if (typeof str !== "string") throw "You can only repeat a string";
  ...
  if (isSpace) {
    ...
    for (let i = 0; i < repeates; i++) repeated += str;
  }
  return repeated;
}
...
function createElement(elementName, attr, val = "", inSideElement = "") {
  let el;
